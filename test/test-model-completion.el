...
(defun test-model-completion-function ()
  ...
  (with-temp-buffer
    (let ((superchat-buffer-name (current-buffer))
      ...
      ;; Test 4: Multiple @ symbols (should only complete the last one)
      (erase-buffer)
      (insert "email@domain @gpt")
      (setq superchat--prompt-start (point-min))
      (goto-char (point-max))
      (let ((result (superchat--completion-at-point)))
        (if result
            (let ((start (nth 0 result))
                  (candidates (nth 2 result)))
              (message "✅ Test 4 passed: Multiple @ symbols handled correctly")
              (message "  Completion starts at: %d (should be after space)" start)
              (message "  Candidates: %S" candidates))
          (message "❌ Test 4 failed: No completion for multiple @ symbols")))))))
