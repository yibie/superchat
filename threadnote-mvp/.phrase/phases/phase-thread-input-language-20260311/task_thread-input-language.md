...
task005 [x] 场景:用户使用 Rime 等输入法连续组词时不应触发输入框高 CPU | Given:IME marked text 会在候选阶段高频刷新且当前输入框每次变更都会同步全量高亮、补全检测与状态回写 | When:在 marked text 组合期间暂停昂贵的编辑器同步并仅在提交后统一刷新 | Then:中文输入阶段不会再把输入法线程和主编辑链路持续拉高到异常占用 | 验证:手动测试+rime 组词输入+swift build
