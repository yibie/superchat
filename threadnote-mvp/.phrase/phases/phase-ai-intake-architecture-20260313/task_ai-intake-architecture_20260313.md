...
task035 [x] 场景: 本地 Ollama 冷启动与大模型请求不再被过紧超时误杀 | Given: native `/api/chat` 在模型未预热或模型较大时可能长时间加载 | When: 用户测试连接或触发 route/resume/draft | Then: 本地 provider 使用更宽松的超时预算、请求级 `keep_alive` 和统一默认模型配置，并在超时时返回可诊断错误 | 验证: `swift test --filter LLMProviderTests`
task036 [x] 场景: 本地推理 prompt 预算收紧且请求串行执行 | Given: resume/route 当前向本地模型发送过肥上下文且 AI 队列允许并发 2 | When: 用户触发 route 或打开多个 thread 生成 restart note | Then: route 只发送最相关的少量候选、resume snippet 预算显著收紧并暴露 prompt 统计，本地 provider 仅串行执行一个推理请求 | 验证: `swift test --filter StoreRoutingTests`
