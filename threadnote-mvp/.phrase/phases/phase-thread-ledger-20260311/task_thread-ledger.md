...
task010 [x] 场景:用户在 thread 内查看和修正工作结果状态，而不会把 `decided/solved/verified/dropped` 混进 entry kind 或主卡片 | Given:当前 Electron clean-room 需要结构化 `entry.status`，且 restart / retrieval 还未正式读取这些 outcomes | When:新增 `entry.status/statusMetadata`、后台状态复判链、手动 `updateEntryStatus` 调用链，并将非 open outcomes 收敛进 `Restart Note` 与 resume / retrieval 上下文 | Then:thread 可在 restart 侧栏聚合展示 Decisions/Solved/Verified/Dropped，用户可在该处手动修正状态，resume 与 retrieval 会把这些 settled outcomes 当辅助信号，EntryCard 继续只展示 kind | 验证:`node --test tests/clean-room/threadnoteAIService.test.js tests/clean-room/applicationService.test.js tests/clean-room/persistence.test.js` + `npx vitest run tests/renderer/useWorkbench.test.jsx tests/renderer/threadNavigation.test.jsx`
