...
task037 [x] 场景: 大 thread 的 AI 上下文改为数据库驱动 | Given: 单 thread 可能包含 1000+ notes + 当前 `resume/prepare` 仍主要依赖 `openThread()` 后的固定 recent slices | When: 为 resume/prepare 增加 query-specific retrieval pipeline，并禁止从前端分页状态推导 prompt 材料量 | Then: AI 输入由 semantic/episodic/source/raw fallback 四层数据库召回决定，页面加载多少不会改变 AI 可见上下文边界 | 验证: `node --test --test-concurrency=1 tests/clean-room/applicationService.test.js` + `npm run test:perf`
task038 [x] 场景: 大 thread 的 counts/status/fingerprint 切到 thread aggregate 账本 | Given: `fetchThreadCounts()` 与 `fetchThreadFingerprintBasis()` 仍会对原始表做 thread-scoped 统计 | When: 引入 `thread_aggregates` materialized ledger，并在 `saveEntry/saveClaim/saveAnchor/deleteEntries` 后单-thread 重建 aggregate | Then: 读路径直接命中账本，fingerprint 与 header 不再对 `entries/claims/anchors` 做实时计数；账本损坏时仍可从原始事实重建 | 验证: `node --test --test-concurrency=1 tests/clean-room/persistence.test.js` + `node --test --test-concurrency=1 tests/clean-room/applicationService.test.js` + `npm run test:perf` + `npm run benchmark:large-thread:record`
task039 [x] 场景: thread aggregate 从整条 thread 重算推进到 patch 更新 | Given: task038 已把读路径切到账本，但同步写路径仍在每次写后单-thread 重算 aggregate | When: 为 entry/status/common claim/common anchor 写入引入 patch-based aggregate update，只在 timestamp 最大值失真等边界回退单-thread rebuild | Then: 常见写入恢复到 O(1) 级记账，`saveEntryWithKnowledgeSync` 不再因为 aggregate 账本而明显回升，同时 bootstrap/repair 路径仍能完整重建 aggregate | 验证: `node --test --test-concurrency=1 tests/clean-room/persistence.test.js` + `node --test --test-concurrency=1 tests/clean-room/applicationService.test.js` + `npm run test:perf` + `npm run benchmark:large-thread:record`
task040 [x] 场景: thread 首开不再默认全量读取整条 memory | Given: `openThread()` 仍会把 thread 下全部 `memory_records` 一次性读出，导致大 thread 首开固定成本偏高 | When: 将 thread detail 改为返回 memory preview + full memory count，并让 inspector/header 使用总数而不是依赖全量数组长度 | Then: thread 首开不再因全量 memory 拉高延迟，Memory 面板仍可显示“总数 + 当前预览”而不破坏现有阅读路径 | 验证: `node --test --test-concurrency=1 tests/clean-room/applicationService.test.js` + `npm run benchmark:large-thread:record`
